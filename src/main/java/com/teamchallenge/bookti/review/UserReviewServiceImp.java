...
public class UserReviewServiceImp implements UserReviewService {
  ...
  public List<UserReview> findAllUserLeftReviewById(Integer userId) {
    if (!userRepository.existsById(userId)) {
      log.info("Throw UserNotFoundException with userId: {}", userId);
      throw new UserNotFoundException(String.format(UserConstant.NOT_FOUND_MESSAGE, userId));
    }
  ...
  public UserReviewResponseDTO save(UserReviewSaveDTO review, Integer reviewerId) {
            ...
            .orElseThrow(
                () ->
                    new UserNotFoundException(
                        String.format("Reviewer's User with id <{%d}> not found.", reviewerId)));

    User userOwner =
        userRepository
            .findById(review.getOwnerId())
            .orElseThrow(
                () ->
                    new UserNotFoundException(
                        String.format(
                            "Owner's User with id <{%d}> not found.", review.getOwnerId())));
    ...
    List<UserReview> userReviewList =
        userReviewRepository.findAllUserReceivedReviewsById(userOwner.getId());

    UserProfileDTO userProfileDTO = UserProfileDTO.mapFrom(userWithNewRating);

    UserReviewResponseDTO userReviewResponseDTO =
        reviewMapper.toUserReviewResponseDTO(userReviewList, userProfileDTO);
    log.info(
        "From UserReviewServiceImp method - save - Create new User Review to user: {}.",
        userWithNewRating.getId());
  ...
  public User addUserRating(User userOwner) {
    BigDecimal userRating =
        userReviewRepository.getUserRating(userOwner.getId()).orElse(BigDecimal.valueOf(0.0));
    userOwner.setRating(userRating.setScale(1, RoundingMode.HALF_UP));
    User userWithNewRating = userRepository.save(userOwner);
    log.info(
        "From UserReviewServiceImp method - addUserRating - Add new user rating: {}; userId: {}.",
        userRating,
        userOwner.getId());
